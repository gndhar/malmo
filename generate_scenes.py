...
def sigma_from_depth(
    z: float, sigma_min: float = 0.5, depth_scale: float = 0.3
) -> float:
...
def gaussian_blob(
    canvas: np.ndarray, cx: float, cy: float, sigma: float, intensity: float
) -> None:
...
def main():
    print(
        f"Generating {NUM_SCENES} scenes of size {IMG_SIZE}×{IMG_SIZE} "
        f"into {OUT_DIR} ..."
    )
